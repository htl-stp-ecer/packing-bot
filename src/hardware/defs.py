...
class Defs:
    ...
    front_left_motor = Motor(port=1, inverted=False)
    front_right_motor = Motor(port=0, inverted=True)
    rear_left_motor = Motor(port=2, inverted=False)
    rear_right_motor = Motor(port=3, inverted=True)
    shild = ServoPreset(
        Servo(port=0),
        positions={
            "up": 65,
